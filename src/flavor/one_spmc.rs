...
unsafe impl<T: Send> Sync for OneSizeSpmc<T> {}
unsafe impl<T: Send> Send for OneSizeSpmc<T> {}

impl<T> OneSizeSpmc<T> {
    #[inline]
    pub fn new() -> Self {
    ...
    fn _pop(&self, order: Ordering) -> Option<T> {
        ...
        loop {
            ...
            match self.pos.compare_exchange_weak(pos, new_pos, SeqCst, SeqCst) {
                Err(_pos) => {
                    pos = _pos;
                }
                Ok(_) => {
...
impl<T: Send + 'static> FlavorImpl<T> for OneSizeSpmc<T> {
    #[inline(always)]
    fn len(&self) -> usize {
        if self.is_empty() {
            0
...
impl<T: Send + 'static> FlavorPrivate<T> for OneSizeSpmc<T> {
    #[inline]
